...
export function matchCategory(item, categoryId) {
    ...
    if (map[categoryId]) return tag === map[categoryId]
    return true
}

function normalizePoem(raw, meta) {

    const tag = meta.tag || raw.tag || ''
    const rawContent = raw.paragraphs || raw.content || []
    const authorDisplay = meta.author || raw.author || '佚名'

    return {
        id: meta.id,
        title: meta.title || raw.title || raw.rhythmic || '未命名',
        author: authorDisplay,
        dynasty: meta.dynasty || raw.dynasty || dynastyFromTag(tag) || '',
        content: rawContent,
        rhythmic: raw.rhythmic,
    }
}

export { dynastyFromTag }
